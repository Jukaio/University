...
static GLuint debug_font_texture_handle = 0;

static void
set_pixel(int width, int height, uint32 *dst, int x, int y, uint32 src)
{
...
static void
blit_glyphs(int width, int height, uint32 *bitmap)
{
   // Source:
...
void win32_create_debug_font()
{
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 128, 128, 0, GL_RGBA, GL_UNSIGNED_BYTE, bitmap);
   glBindTexture(GL_TEXTURE_2D, 0);

   debug_font_texture_handle = id;
}

namespace Helium
{
   void Renderer::draw_text(const Point &point, const Color &color, const char *format, ...) const
   {
      char text[1024] = {};
      va_list args;
      va_start(args, format);
      int len = vsprintf_s(text, sizeof(text) - 1, format, args);
      va_end(args);
      ...
      glBindTexture(GL_TEXTURE_2D, debug_font_texture_handle);
      ...
      for (int index = 0; index < len; index++)
      {
         ...
         glTexCoord2f(s, v); glVertex2i(x + character_width, y                  );
         glTexCoord2f(s, t); glVertex2i(x + character_width, y + character_width);
         glTexCoord2f(u, t); glVertex2i(x                  , y + character_width);
         glEnd();
